...
try {
  ...
  const device = (await callJson('get_device', {
    device_id: deviceId,
  })) as DeviceSummary;
  assert(
    device.generated_id === deviceId,
    'get_device should return the requested device',
  );
  ...
  assert(
    'timestamp' in readings[0],
    'normalized public API reading should include timestamp',
  );
  ...
  assert(
    comparison[0].device_id === deviceId,
    'compare_devices should include requested device',
  );
