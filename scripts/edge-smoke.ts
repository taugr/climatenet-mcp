...
try {
  ...
  assert(Array.isArray(shirakOnline), "filtered list_devices should return an array");
  assert(shirakOnline.every((device: any) => device.parent_name_en === "Shirak"), "region filter should apply");
  assert(shirakOnline.every((device: any) => device.Status === "online"), "status filter should apply");
  assert(shirakOnline.every((device: any) => device.PMS5003 === "invalid"), "sensor filter should apply");
  ...
  await expectToolError("get_device_readings", {
    device_id: 8,
  ...
  assert(Object.keys(metricGraph[0]).join(",") === "time,value", "metric graph should reduce to time/value points");
